...
public:
	void param_chaeck()
	{
		if (shape_model == NULL)
		{
			std::cout << "netp1 shape_model が設置されていません" << std::endl;
		}
		if (texture_model == NULL)
		{
			std::cout << "netp1 texture_model が設置されていません" << std::endl;
		}
		if (shape_proc_train == NULL)
		{
			std::cout << "netp1 shape_proc_train が設置されていません" << std::endl;
		}
		if (texture_proc_train == NULL)
		{
			std::cout << "netp1 texture_proc_train が設置されていません" << std::endl;
		}
		if (shape_proc_test == NULL)
		{
			std::cout << "netp1 shape_proc_test が設置されていません" << std::endl;
		}
		if (texture_proc_train == NULL)
		{
			std::cout << "netp1 shape_proc_train が設置されていません" << std::endl;
